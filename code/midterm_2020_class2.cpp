...
void print_data(const unsigned char* pkt_data, int caplen) {
	...
	if (caplen > except_data_len) {
		int i;
		for (i = except_data_len; i < caplen; i++) {
			if (pkt_data[i] == 'p' && pkt_data[i + 1] == 'a' && pkt_data[i + 2] == 's'
				&& pkt_data[i + 3] == 's' && pkt_data[i + 4] == 'w' && pkt_data[i + 5] == 'd'
				&& pkt_data[i + 6]== '=') {
				i = i + 6;
				printf("found passwd=");
				for (;;) {
...
int main() {
	...
	if (pcap_compile(fp,					// pcap handle
		&fcode,							// compiled rule
		(char*)("host 165.246.38.157 and port 80"),	// filtering rule
		1,								// optimize
		NULL) < 0) {
